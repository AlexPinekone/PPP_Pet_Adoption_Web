<?php
	...
	if(isset($_POST['enviar']))
		...
		if (isset($_POST['message'])) {
			...
			if($cad->registrarMensaje($idChat,$message,$deQ))
			{
				$idU= $_SESSION['idU'];
				$idD= $_SESSION['idD'];
				header("Location: chat.php?idU=$idU&idD=$idD");
			}
	...
	$mascotaAct = [];
	$mascotaAct = $_SESSION['mascotaAct'];
...
<html>
	...
	<body>
		<div class="Principal">
			...
			<div class="Espacio2"></div>
				...
				<?php
					...
					$datosDueno = $cad->buscaUsuario($idD);
					$datosMasc = $cad->obtenerMascota($mascotaAct);
					$dm = $datosMasc[0];
				?>
				<div class="ContHori">
				<div class="Mascota">
				...
				<div class="pelillo">>> Hablando sobre >></div>
				<div class="Mascota">
					<a href="<?php echo 'postx.php?id='.$dm ?>">
						<?php
							echo "<img src='data:image/;base64,".base64_encode($datosMasc[7])."' alt='Mascota'>";
						?>
						<p><?php echo $datosMasc[2]; ?></p>
					</a>
				</div>
				</div>
		...
		?>		
					...
					<form id="enviaMensaje" action="chat.php" method="POST">
