<?php
	...
	if(isset($_GET['termino']))
	{
		...
		$mascotas = $cad->buscarMascotas($termino);
		
		if (is_array($mascotas) && count($mascotas) > 0) {
			foreach ($mascotas as $mascota) {
				$urls[$mascota["idPost"]] = "postx.php?id=" . $mascota["idPost"];
			}
		}
	}
	...
	if(isset($_GET['etiqueta']))
	{
		...
		$mascotas = $cad->buscarMascotasEtiqueta($eti);
